  ...
  return <main className="admin-page">
    <div className="admin-shell">
      ...
      <section className="admin-main">
        ...
        <div className="admin-kpis"><article><small>Total sales</small><strong>{formatPrice(revenue)}</strong><span>From recorded orders</span></article><article><small>Orders</small><strong>{stats?.orders ?? orders.length}</strong><span>Customer orders</span></article><article><small>Products</small><strong>{stats?.products ?? products.length}</strong><span>Catalogue items</span></article><article className={(stats?.lowStock ?? lowStock.length) ? 'warning' : ''}><small>Low stock</small><strong>{stats?.lowStock ?? lowStock.length}</strong><span>Needs attention</span></article></div>
        <div className="admin-grid-two">
          <section className="admin-panel"><div className="admin-panel-head"><div><span className="eyebrow">RECENT ACTIVITY</span><h2>Recent orders</h2></div><Link to="/admin/orders">View all →</Link></div>{orders.length ? <div className="admin-table">{orders.slice(0, 5).map((order) => <div className="admin-order-row" key={order.id}><div><b>#{order.id}</b><small>{order.customer?.name || 'Customer'} · {new Date(order.createdAt || order.date).toLocaleDateString('en-NG')}</small></div><span className={`admin-status ${String(order.status).toLowerCase()}`}>{order.status}</span><strong>{formatPrice(Number(order.total || 0))}</strong></div>)}</div> : <div className="admin-empty">No customer orders have been recorded yet.</div>}</section>
        ...
        <div className="admin-demo-note">Dashboard metrics and order records are now loaded from the backend database.</div>
      </section>
    </div>
  </main>
}
