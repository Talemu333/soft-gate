...
import { categories as seedCategories, formatPrice } from '../data/products'
...
export default function AdminProducts() {
  ...
  const remove = (id) => {
    if (window.confirm('Remove this product from the demo catalogue?')) removeProduct(id)
  }
  ...
  const submit = (event) => {
    ...
    const nextProduct = {
      ...
      image: editing?.image || products[0]?.image,
      description: data.get('description'),
      specs: editing?.specs || [],
    }

    if (editing?.id) updateProduct(editing.id, nextProduct)
    else addProduct(nextProduct)
    setEditing(null)
  ...
  return <main className="admin-page"><div className="admin-shell"><AdminNav active="products"/><section className="admin-main"><header className="admin-header"><div><span className="eyebrow">CATALOGUE & INVENTORY</span><h1>Products</h1><p>Manage products, pricing and stock levels.</p></div><button className="primary-button" type="button" onClick={() => setEditing({})}>+ Add product</button></header>
    ...
    {editing && <div className="admin-modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && setEditing(null)}><form className="admin-modal" onSubmit={submit}><div className="admin-modal-head"><div><span className="eyebrow">{editing.id ? 'EDIT PRODUCT' : 'NEW PRODUCT'}</span><h2>{editing.id ? 'Update product' : 'Add product'}</h2></div><button type="button" onClick={() => setEditing(null)}>×</button></div><div className="form-grid"><label>Product name<input name="name" defaultValue={editing.name || ''} required /></label><label>Category<select name="category" defaultValue={editing.category || seedCategories[1]}>{seedCategories.slice(1).map((name) => <option key={name}>{name}</option>)}</select></label><label>Price<input name="price" type="number" min="0" defaultValue={editing.price || ''} required /></label><label>Old price<input name="oldPrice" type="number" min="0" defaultValue={editing.oldPrice || ''} /></label><label>Stock<input name="stock" type="number" min="0" defaultValue={editing.stock ?? ''} required /></label><label>Badge<input name="badge" defaultValue={editing.badge || ''} placeholder="New, Popular..." /></label></div><label>Description<textarea name="description" defaultValue={editing.description || ''} required /></label><button className="primary-button full" type="submit">{editing.id ? 'Save changes' : 'Add product'} →</button></form></div>}
  </section></div></main>
}
